...
@cython.boundscheck(False)
def slice_entropy(np.ndarray[np.int32_t, ndim=1] y, SIZE_t start, SIZE_t end):
...
@cython.boundscheck(False)
cdef bint reject_split(np.ndarray[np.int32_t, ndim=1] y, int start, int end, int k):
...
@cython.boundscheck(False)
def find_cut(np.ndarray[np.int32_t, ndim=1] y, int start, int end):
